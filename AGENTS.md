...
1. **同一时间整个仓库只允许一路 Agent 写**（2026-09-20 用户定的"单写者"）。另一路要并行就只能**只读**（查代码、跑分析、出报告），不许编辑、不许 `git checkout`/`switch`、不许跑全量门禁。
   为什么这么严：两路共用**同一个工作树**，`git switch`/`checkout` 会互相抽底 —— 2026-09-20 一路会话用 `git checkout -- .` 撤回自己的改动时，差点吞掉另一路的未提交改动，对方只是恰好先 commit 了才没出事。分支救不了这个（工作树是共享的），真隔离要 `git worktree`，但那样 `CHANGELOG.md` 会变成两份、锁和看板同时失效。
2. 同一份文件层面仍然要锁：要改之前先在 `CHANGELOG.md` 上锁；改完释放。
3. 需要动别人刚写的逻辑：先在 `CHANGELOG.md` 写清理由，锁住相关文件，改动面尽量小。
4. 不做「顺手大重构」。一次改动只服务一个可验证目的。
5. 未分配任务的 Agent，不碰职责范围外的文件（见第 2 节角色）。
6. 沟通只写进 `CHANGELOG.md`，**不要埋在代码注释里**（注释只解释代码本身为什么这样写）。
...
```bash
cd publish
node tests/run.js            # 零依赖单测（条数一直涨，以实跑输出为准），必须 100% 通过
...
- 页面脚本与弹窗**没有假 DOM 可测**（`tests/harness.js` 只注入 window），
  ...
  教训（2026-09-19）：`task.js` 调了一个本文件不存在的 `renderCount()`，当时那 500 多条单测全绿，只有真浏览器一点才炸。
...
1. 本次改动的文件清单（含新增/删除）；
2. 改动摘要（做了什么、为什么这样做）；
3. 门禁结果原话（`NNN/NNN 通过`、`ALL PASS` 这类，不能只说「测过了」；数字一律按实跑填）；
