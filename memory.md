...
- **常用查询的「人」与「部门」键（2026-09-23 用户拍板）**：归属键**工号优先**，但按人查询**允许工号↔姓名互查**
  （`lib/queries-db.js` 的 `byUser` 同比 `user_key/user_id/user_name`；前端 `saverKeysOf` 也把 owner 的工号**和**姓名
  都算进键集合，否则服务端放宽了会在本机过滤这一步又被抹掉）。「当前用户」缺工号或缺 team 级部门时
  **必须当场提醒**（`CurrentUser.missingOf` / `incompleteWarning`），角标走 `'incomplete'` 显示**「身份不全」**、
  不许说「已同步」；部门榜标题只有在真拿到服务端 `?dept=` 结果时才准写部门名，否则带「（本机）」——
  这与上面「别为刷新及时性改角标文案」不冲突：那条讲实时性，这条讲**口径真假**（把本机说成部门、把查不到说成已同步，都是谎报）。
