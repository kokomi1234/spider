/**
 * Mock 代理服务器（**手动启动的调试后端**，端口 3001）
 *
 * ⚠️ 它不在三个页面的 <script> 列表里，也不是页面的运行依赖 —— 只在需要「完全不碰内网、
 * 也不依赖抓包缓存」时手动 `node tools/mock-proxy.js` 起来。日常离线开发请用
 * `PROXY_OFFLINE=1 node proxy.js`（回放 publish/cache/ 里的真实响应），那才是主路径。
 * 保留它的原因：真实接口与缓存都没有时，还能把页面点起来看布局/交互。
 */
