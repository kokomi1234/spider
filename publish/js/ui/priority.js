 ...
 *
 * 例：2609批次 → 2026-08-15 前转功能测试基线，2026-09-15 前转正式版基线。
 * 与「批量修改批次时间」选择栏的默认值口径一致（功测 = 批次月的上一个月）。
 * 改规则只动下面 MILESTONES / LEVELS 两处。
 *
...
(function () {
  ...
  const MILESTONES = [
    { from: '开发基线',     to: '功能测试基线', offsetMonth: -1, day: 15, dateField: 'testDate' },
   ...
   *
   * 月份相关的两个坑在这里一起堵掉：
   *   · 跨年/跨月进位交给 Date（month 传 0 / 13 / -1 都合法）：2601批次 -1 个月
   *     要落到上一年的 12 月，手写 `month - 1` 再减年会算错。
