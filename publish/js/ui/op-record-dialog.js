 ...
 * · operationType 是**数字编码**：已确认的 5 个映射写在 publish-dialog-model.js 的
 *   OP_TYPES 里；没确认的编码**原样显示数字**，不猜中文（筛选下拉同理，
 *   只列编码已知的项 —— 编码填错会让筛选静默查错数据）。
 *
 * ── 对外 ──────────────────────────────────────────────────
...
(function () {
  ...
  function ensureInit() {
    ...
    // 选了操作类型 → **只清掉旧结果，不自动发请求**（这一栏有「查 询」按钮，
    // 自动查会与用户点按钮的动作重复）。但必须清空列表：否则用户改了条件
    // 却对着上一份结果，会以为那就是新条件的结果。
    // searchable-select 在「选中」时会向宿主 <select> 派发冒泡 change，这条接得住；
    // clear() / setValue() 是静默的，所以程序化复位不会误触发重查。
    const sel = $('#opTypeFilter');
    if (sel) sel.addEventListener('change', () => { rows = []; total = 0; renderAll(); });
