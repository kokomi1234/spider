...
(function () {
   ...
   * 且不报错 —— 要动这批先回来读这段。
   *
   * ── 反推不出来、**故意留空**的 9 个名字（别硬凑一个编码填上）────────
   * · `删除` / `接口批量更新` / `发布` / `取消发布`：四个名字挤在 22~25 四个编码里，
   *   能确定的只有**组内相对顺序**（删除 < 接口批量更新、发布 < 取消发布），
   *   满足条件的排法有 6 种 → 定不了。最像的一种是
   *   22 删除 / 23 发布 / 24 取消发布 / 25 接口批量更新
   *   （理由：20/21/22 像 CRUD 三连；发布↔取消发布、接口批量更新↔接口批量新增 各自成对、
   *   成对者编码相邻），但**这只是"最像"，没有证据**，所以不写。
   * · `取消订阅时上一批量新增订阅关系审批流程接口材料移除` / `UNCHECK` / `服务订阅-删除`：
   *   落在 64~74 之间，顺序能定、具体编码定不了（后面还跟着 75）。
