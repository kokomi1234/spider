...
### Token 归属：本机 token + 管理员兜底（2026-09-22）

token **只存在使用者自己的浏览器里**（`localStorage` 的 `spider.userToken.v1`），
发请求时随请求头 `x-user-token` 带给代理转发 —— **代理侧不保存任何用户凭证**
（用户明确要求：有的同事 token 权限较高，不愿交给后端存）。
（早前一版曾按工号存代理侧 SQLite；同日按用户要求整体撤掉，
`lib/user-tokens.js` 与 `shared/user-tokens.db` 已删除。）

| 情况 | 用谁的 token | 能做什么 |
|---|---|---|
| 本机录入过（**没登录也能录**） | 就这台机器上那份 | 全套操作 |
| 没录 / 录了但过了 5:00 | 管理员 token（`.env` 的 `PROXY_TOKEN`） | **只有查询权限** |
| 管理员本人（`.env` 的 `ADMIN_USER_ID`，默认 4711510） | 同样回落管理员 token | 全套操作（他本来就是这个身份） |

- **过期**：内网 token 每天早上 **5:00** 失效。判定放在前端（token 就在本地）：
  「录入时间早于最近一次 5:00」就算过期 —— 过期的不会被带出去，省掉一次必然 401 的请求。
  5 点之后重新录入**立刻生效**。
- **受限的功能**：只有**订阅**（写内网 `POST /itamp-tool/publish/setSubcription`）；
  **取消订阅/删除照旧可用**（那个接口本来就是空的，只删本机标记）。
  界面上订阅按钮会置灰并写明原因，点击时再拦一道。
- 「本次用谁的」由**前端自己算**（`API.tokenSource()` → `user` / `admin` / `fallback`），
  不依赖响应头往返 —— 离线回放时也算得准。代理仍回 `x-token-source`，仅作诊断。
- 录入与清除都在右上角「🔑」那个弹窗里（标题随身份变：本机 Token / 管理员 Token），
  清除有二次确认；状态区两行：管理员 token + 本机 token。
- ⚠️ `fallback` 与 `admin` 别混：管理员本人用的也是管理员 token，但他有全套权限 ——
  混起来会把管理员自己的订阅也禁掉。
...
2026-09-20 改）。没设当前用户时显示**本机那些「无人认领」的**记录（就是没登录时保存的），
有归属人的（同事的）一条都不露 —— 「宁可不显示，也不把同事的查询说成你的」这条没破。
等设了「当前用户」，它们会被**自动认领**到名下（2026-09-22 加 `claimAnonymous`）：
以前是空列表 + 让用户"再去查询页重新保存一次"，等于让人白存一次。
本机镜像**分两个存储键**（同日）：登录镜像段 `spider.savedQueries.v1`（会被服务端同步整段覆盖）
与匿名段 `spider.savedQueries.anon.v1`（登录/退出都不碰）—— 否则登录一次就把没登录时存的冲掉了。
所以**换人列表就会换**；要按部门看别人的，看下面的「部门常用查询」。
**导出 / 导入也是按人的**（2026-09-21 用户拍板，此前误取团队库全集）：
导出导**当前用户自己的**记录，导入进来的记录**归当前用户**（按 id / 同页面同名合并）。
没设当前用户时两侧都只走本机镜像 —— 那时的常用查询保存在 localStorage、不进共享库。
（要看别人的用下面的「部门常用查询」，不要靠导出。）
