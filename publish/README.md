...
### Token 归属：管理员兜底 + 用户各自录入（2026-09-22）

以前 token 是**全局一个**（`.env` 的 `PROXY_TOKEN`），所有人共用一个身份。现在按人：

| 身份 | 用谁的 token | 能做什么 |
|---|---|---|
| 管理员（`ADMIN_USER_ID`，默认 `4711510`；配在 `.env`） | `.env` 里的 `PROXY_TOKEN`（由运维工具每天自动刷新） | 全套操作 |
| 登录用户·录入过自己的 token | 他自己的（存 `shared/user-tokens.db`，绑到工号） | 全套操作 |
| 登录用户·没录入 / 录了但**过期** | 回落到管理员 token | **只有查询权限** |
| 没登录 | 回落到管理员 token | **只有查询权限** |

- **过期规则**：内网 token **每天早上 5:00** 失效 —— 判定就是「录入时间早于最近一次 5:00」。
  所以 5 点后重新录入**立刻生效**，不用等第二天。
- **受限的功能**（用户拍板）：只禁**订阅**（写内网 `POST /itamp-tool/publish/setSubcription`）；
  **取消订阅/删除仍然可用**（那个接口本来就是空的，只删本机标记）。
  界面上订阅按钮会置灰并写明原因，点击也会拦一道。
- 实现接口：前端每次请求带 `x-user-key`（当前工号，未登录不带），代理据此选 token 并把结论
  放进响应头 `x-token-source`（`user` / `admin` / `fallback`）；页面据此更新「🔑」按钮的文案
  （`🔑 我的 token` / `🔑 管理员` / `🔑 管理员 token`）。
- 录入入口：就是原来的「🔑 Token」弹窗，按身份切换语义 —— 管理员改全局 token（可写 `.env`），
  普通用户录自己的（不碰 `.env`）；未设当前用户时不允许改（免得动到全局那个）。
- ⚠️ `x-token-source` 三个取值别混：`admin`（管理员本人）和 `fallback`（别人回落）都"在用管理员
  token"，但前者有全套权限、后者只读 —— 混起来会把管理员自己也锁掉。
