...
- [ ] **缺全局异常兜底（`window.onerror` / `unhandledrejection` 全项目 0 处）**：目前三页都没有
     全局错误捕获，也未在 HTML 内联。**未捕获的 Promise rejection 会静默失败——用户看到的就是"点了没反应"**，
     而这正是本项目最难自查的故障形态（记忆里"页面没反应先怀疑缓存没命中"就是被它坑出来的）。
     **目标**：任何未捕获异常都能在界面上给出提示 + 控制台留痕，别再静默吞。
     **卡点**：需注意别把无害噪音（如 ResizeObserver、图片 404）也弹成 toast 骚扰用户。
     **需要**：在 bootstrap.js 里加兜底（建议：控制台必记，toast 只对 Error 级弹），
     并按 `?debug=1` 分级。三页都加载 bootstrap，一处即可覆盖。
